...
- ⚠️ **Bug trovato e corretto in un audit successivo** (`20260718100000_fix_chain_swap_buyer_direction.sql`): `confirm_chain_participant()` registrava il `buyer_id` sbagliato in `transactions` per ogni scambio a 3 completato — calcolava il destinatario nella direzione opposta a quella validata da `create_chain_proposal()`. Solo lo storico ("Attività > Storico") era affetto, non la UI della catena in corso. Confermato su dati reali (3 righe storiche corrette) e non solo in teoria.
...
- ✅ **Email di servizio: SMTP → Resend**: Render blocca l'SMTP in uscita su entrambe le porte comuni (465/587) — verificato in produzione dopo due tentativi di fix lato client (selezione IPv6, `family:4`) che non risolvevano nulla, perché il blocco è della piattaforma di hosting, non un problema di indirizzo/porta. Sostituito `lib/mailer.js` con l'API HTTPS di Resend (porta 443, mai bloccata). Il dominio di test `onboarding@resend.dev` consegna solo al proprio indirizzo verificato — per mandare a utenti reali serve un dominio proprio verificato su Resend (SPF/DKIM), non ancora fatto.
- ✅ **Link "pausa/elimina" nell'email di segnalazione**: prima l'email a chi modera (`REPORT_NOTIFY_TO`) era solo testo informativo, senza modo di agire se non aprendo il Table Editor di Supabase a mano. Aggiunto un token monouso per segnalazione (`report_action_tokens`, scadenza 7 giorni, stesso pattern di `fb_link_codes`) con due link nell'email: pagina di conferma via GET (non consuma nulla), azione vera solo su POST innescato dal bottone — per evitare che un client email o uno scanner che pre-carica i link in automatico esegua l'azione da solo (rischio concreto con "elimina", stato terminale per un annuncio).
- ✅ **"Compila con AI" deduce l'operatore anche dal solo tipo di servizio**: il campo `operator` (colonna già esistente, mai chiesto a mano) restava vuoto se il testo nominava solo "Frecciarossa"/"Intercity" senza mai dire "Trenitalia". Aggiunta euristica al prompt: marchi commerciali esclusivi di un operatore bastano da soli (Frecciarossa/Frecciargento/Freccia Bianca/Intercity/Intercity Notte/EuroCity/Euronight → Trenitalia; Italo → Italo). "Regionale"/"Regionale Veloce" restano esclusi apposta: gestiti da aziende diverse per regione, un operatore sbagliato sarebbe peggio di uno mancante.
- ✅ **Prezzo dinamico**: un annuncio VENDO vale il prezzo pieno fino a un attimo prima della partenza/check-in, poi vale zero — a differenza di un marketplace generico qui il "deperimento" è certo e calcolabile. Il venditore può attivare, per singolo annuncio, uno sconto automatico che scende linearmente negli ultimi giorni prima dell'evento (default 7, `PRICE_DECAY_WINDOW_DAYS`) fino a un minimo che decide lui. Nuove colonne `dynamic_pricing_enabled`/`price_floor`/`list_price`, cron `POST /api/price-decay/recompute` (aggiorna solo `price`, mai un aumento — `min(prezzo corrente, target)`), notifica in-app ad ogni scatto. Toggle in creazione/modifica annuncio, badge nel dettaglio.
- ✅ **Fix: un ricontrollo AI fallito cancellava un'affidabilità già valida**: modificando un annuncio pubblicato e già verificato (es. cambiando le foto, che forza un nuovo Check AI), se quella nuova verifica falliva o restava "in sospeso" (chiave OpenAI assente, 429, timeout), il trigger `update_listing_trust_score` scriveva comunque `trust_score = NULL` sopra un punteggio buono già mostrato — l'affidabilità spariva del tutto dall'annuncio, senza che nulla di reale fosse cambiato. Corretto in `20260731150000`: un ricontrollo fallito non tocca più un punteggio precedente valido; il ramo "in sospeso" resta solo per un annuncio mai verificato prima (il caso per cui `trust_pending_at` era stato introdotto).
- ✅ **In modifica, prezzo/tratta/date forzano di nuovo il Check AI**: prima, in modalità modifica, SOLO le foto invalidavano la verifica precedente — cambiare prezzo, tratta o date lasciava visibile un'affidabilità calcolata su dati non più reali, senza alcun avviso. Aggiunto `criticalFieldsDirtySinceCheck` (stesso meccanismo già usato per le foto) per questi campi specificamente antifrode; titolo/descrizione/altri campi secondari restano volutamente esclusi, per non intralciare piccole correzioni di testo.
- ✅ **Proposte che non morivano con l'annuncio**: una proposta restava "in attesa di risposta" per sempre se l'annuncio dall'altra parte scadeva — e chi l'aveva fatta non veniva avvisato di nulla. `expire_my_stale_listings` ora scade anche le proposte pendenti agganciate a un annuncio non più disponibile, e `notify_on_offer` ha un ramo `expired` che distingue i due casi ("l'annuncio non c'è più" vs "è scaduta senza risposta"), perché sono due cose diverse per chi le riceve.
- ✅ **Classe, tariffa e reintestabilità del biglietto**: dall'import si estraggono anche classe di servizio e tipologia di tariffa (cose distinte: un biglietto può essere Business con tariffa Economy), e da lì si ricava se il nome è cambiabile. La mappa tariffa → operatore è **dati** (`lib/fareRules.js`), non un giudizio del modello, con `UNKNOWN` quando non si sa; un valore dichiarato dal venditore vince sempre su quello dedotto. Il risultato è un **avviso, mai un blocco**: le politiche cambiano, e impedire una vendita legittima è peggio che avvisare a vuoto.
- ✅ **Avviso prezzo automatico**: la stima parte da sola appena si scrive il prezzo e avvisa **solo verso l'alto** (oltre 1.25× il consigliato). Nessun avviso per un prezzo basso: chi vende a poco lo sta facendo apposta. Gira sul modello economico perché è un semaforo, non un'analisi da leggere parola per parola.
- ✅ **Tracciamento errori (Sentry, regione UE)**: non i crash — quelli erano già nei log — ma i guasti **gestiti**, i `catch` che ricadono su un fallback. Erano invisibili ed erano esattamente ciò che gli utenti vedevano: l'AI che non risponde, l'email che non parte, il cron che salta un giro. `reportFault` li manda con un throttle di 5 minuti per scope+messaggio; l'app fa lo stesso via `/api/client-errors`. Header, corpo e query string ripuliti prima dell'invio.
- ✅ **Import PDF: sei minuti contro i novanta secondi che l'app aspetta**. I retry dell'SDK moltiplicati per i nostri producevano una rotellina infinita senza mai un errore. Client dedicato al PDF (timeout corto, retry SDK disattivati) più un budget complessivo che rinuncia al secondo tentativo se il tempo non basta. Nella stessa area: il credito OpenAI esaurito (`429 insufficient_quota`) ora è riconosciuto, **non** ritentato, e detto all'utente in modo che sappia cosa fare.
- ✅ **Costi OpenAI, quattro interventi misurati sul consumo reale**: cache sulle stime prezzo (chiave = prompt esatto), modello economico dove il testo non si legge, e per le catene a 3 — che erano la voce dominante, 7 centesimi a giro con 333 annunci — due difese deterministiche prima di pagare il modello: si salta l'intero ricalcolo se nulla di rilevante è cambiato, e si scartano i candidati oltre 30 giorni di distanza. In più il modello risponde per **indice** invece che per uuid e senza motivazione (che nessuno leggeva): uscita −78%, ingresso −32%, ed erano gli output token la voce più cara del conto.
  L'impronta "è cambiato qualcosa?" ignora di proposito il **prezzo**: il decadimento automatico lo riscrive di continuo senza spostare un solo ciclo, e guardandolo il ricalcolo non saltava mai — un cron pagava per l'altro.
- ✅ **Test veri contro Postgres**: i test sostituivano il client Supabase con un finto, quindi trigger, vincoli e RLS non venivano mai eseguiti — ed è lì che sono nati due guasti arrivati in produzione con la CI verde (il cast mancante su `offers.status`, il vincolo sui tipi di notifica). Ora `server/tools/apply_migrations.mjs` ricostruisce lo schema da zero (72/72 migration pulite) e 15 test in `server/test/db/` provano le regole dove vivono, ognuno in una transazione annullata. Job CI `test-db` separato; senza `DATABASE_URL` quei test si saltano da soli.
- ✅ **Verifica di allineamento repo ↔ database**: `supabase/verify_schema.sql`, generato dalle migration, elenca ciò che manca nel database (31 tabelle, 264 colonne, 78 funzioni, 20 trigger). Non è teoria: ha trovato **sei migration consecutive del 30 luglio** mai eseguite in produzione — due cron fallivano a ogni giro, le valutazioni degli scambi a 3 e la risoluzione delle dispute semplicemente non esistevano, e nulla lo segnalava. Recuperarle ha richiesto anche di riapplicare in coda la versione più recente di ciò che quelle migration riportavano indietro (vedi `CLAUDE.md`).
